...
export function calculateRetention(
  ...
  let importanceMult = DECAY_CONFIG.IMPORTANCE_MULTIPLIER.NORMAL;
  if (importance < 0.3) {
    importanceMult = DECAY_CONFIG.IMPORTANCE_MULTIPLIER.LOW;
  } else if (importance >= 0.9) {
    importanceMult = DECAY_CONFIG.IMPORTANCE_MULTIPLIER.CRITICAL;
  } else if (importance >= 0.7) {
    importanceMult = DECAY_CONFIG.IMPORTANCE_MULTIPLIER.HIGH;
  }
  
  // Effective strength includes importance
  const effectiveStrength = Math.max(
    DECAY_CONFIG.MIN_STRENGTH,
    strength * importanceMult
  );
  
  // Ebbinghaus formula
  const decayRate = DECAY_CONFIG.DECAY_CONSTANT / (effectiveStrength * 24); // per hour
 ...
 */
export function getRetentionLevel(retention: number): 'strong' | 'moderate' | 'weak' | 'forgotten' {
  if (retention >= DECAY_CONFIG.RETENTION_THRESHOLDS.STRONG) return 'strong';
  if (retention >= DECAY_CONFIG.RETENTION_THRESHOLDS.MODERATE) return 'moderate';
  if (retention >= DECAY_CONFIG.RETENTION_THRESHOLDS.WEAK) return 'weak';
  return 'forgotten';
}
 ...
 */
export function calculateStrengthBoost(
  currentStrength: number,
  accessCount: number,
  importance: number = 0.5
): number {
  // Diminishing returns: boost decreases with more accesses
  const boostFactor = DECAY_CONFIG.RECALL_BOOST.BASE / 
    (1 + DECAY_CONFIG.RECALL_BOOST.DIMINISHING * accessCount);
  
  // Higher importance = bigger boost
  const importanceBoost = 1 + (importance - 0.5) * 0.5;
  
  const boost = Math.min(
    DECAY_CONFIG.RECALL_BOOST.MAX_BOOST,
    boostFactor * importanceBoost
  );
  
  return Math.min(
    DECAY_CONFIG.MAX_STRENGTH,
 ...
 */
export function calculateDecayRate(strength: number, importance: number = 0.5): number {
  let importanceMult = DECAY_CONFIG.IMPORTANCE_MULTIPLIER.NORMAL;
  if (importance < 0.3) importanceMult = DECAY_CONFIG.IMPORTANCE_MULTIPLIER.LOW;
  else if (importance >= 0.9) importanceMult = DECAY_CONFIG.IMPORTANCE_MULTIPLIER.CRITICAL;
  else if (importance >= 0.7) importanceMult = DECAY_CONFIG.IMPORTANCE_MULTIPLIER.HIGH;
  
  const effectiveStrength = strength * importanceMult;
  return DECAY_CONFIG.DECAY_CONSTANT / (effectiveStrength * 24); // per hour
}
 ...
 */
export function getDecayStatus(memory: {
  id: string;
  content: string;
  strength: number;
 ...
 */
export function getDecayStatusSQL(): string {
  return `
    SELECT 
 ...
 */
export function getNeedsReviewSQL(): string {
  return `
    SELECT 
      id, content, strength, last_accessed, access_count, importance
    FROM memories
 ...
 */
export function getAtRiskSQL(): string {
  return `
    SELECT 
      id, content, strength, last_accessed, access_count, importance
    FROM memories
 ...
 */
export function getArchiveSQL(): string {
  return `
    UPDATE memories
 ...
 */
export function getDeleteArchivedSQL(): string {
  return `
 ...
 */
export function getHealthStatsSQL(): string {
  return `
    SELECT
      COUNT(*) as total,
 ...
 */
export function getStrengthDistributionSQL(): string {
  return `
    SELECT
 ...
 */
export function getBoostStrengthSQL(): string {
  return `
    UPDATE memories
    SET 
...
export function generateHealthRecommendations(health: {
  totalMemories: number;
  atRiskCount: number;
  needsReviewCount: number;
  averageStrength: number;
  strengthDistribution: { strong: number; moderate: number; weak: number; forgotten: number };
}): string[] {
  ...
  if (weakPercent > 0.3) {
    recommendations.push(
      `⚠️ ${Math.round(weakPercent * 100)}% of memories are weak/forgotten. Consider reviewing important ones or running cleanup.`
    );
  }
  ...
  if (health.atRiskCount > 10) {
    recommendations.push(
      `🔴 ${health.atRiskCount} memories at risk of being forgotten. Use memory_decay_status filter='at_risk' to see them.`
    );
  }
  ...
  if (health.needsReviewCount > 20) {
    recommendations.push(
      `📝 ${health.needsReviewCount} memories need review. Recall important ones to strengthen them.`
    );
  }
  ...
  if (health.averageStrength < 1.5) {
    recommendations.push(
      `📉 Average strength is low (${health.averageStrength.toFixed(2)}). Increase recall frequency for important memories.`
    );
  }
  ...
  if (health.strengthDistribution.forgotten > 50) {
    recommendations.push(
      `🗑️ ${health.strengthDistribution.forgotten} forgotten memories. Run memory_cleanup to archive them.`
    );
  }

...
export async function searchMemories(
  ...
  const projectOpts: ProjectQueryOptions = {
    projectId: projectId ?? getProjectContext().id,
    includeGlobal: includeGlobal ?? getIncludeGlobal(),
    allProjects,
  };
...
async function searchBM25(
  query: string,
  options: { projectOpts: ProjectQueryOptions; type: MemoryType | undefined; limit: number }
): Promise<SearchResult[]> {
  const db = getDatabase();
  const { projectOpts, type, limit } = options;

  // Build project filter using isolation helper
  const projectFilter = buildProjectFilter(projectOpts, 'm.project_id');
  
  // Build WHERE clause
  let whereClause = `m.deleted_at IS NULL AND ${projectFilter.clause}`;
  const params: unknown[] = [query, ...projectFilter.params];
...
function searchVectorBruteForce(
  ...
  // Build project filter
  const projectFilter = buildProjectFilter(projectOpts, 'project_id');
...
async function searchHybrid(
  query: string,
  options: { projectOpts: ProjectQueryOptions; type: MemoryType | undefined; limit: number; bm25Weight: number }
): Promise<SearchResult[]> {

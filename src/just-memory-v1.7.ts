...
function validateObservations(observations: string[]): string[] {
...
function assessConfidence(memory: MemoryRow): { level: 'high' | 'medium' | 'low' | 'uncertain'; note?: string } {
  ...
  if (conf >= CONFIDENCE_LEVELS.LOW) return { level: 'low', note: 'Limited corroboration.' };
  if (memory.contradiction_count > 0) return { level: 'uncertain', note: `${memory.contradiction_count} conflicting source(s).` };
  return { level: 'uncertain', note: 'Single source, unconfirmed.' };
}

function toConfidentMemory(memory: MemoryRow) {
...
// ============================================================================
function findContradictions(content: string, excludeId?: string, limit = 5): MemoryRow[] {
...
// ============================================================================
function storeMemory(content: string, type = 'note', tags: string[] = [], importance = 0.5, confidence = 0.5) {
...
function updateMemory(id: string, updates: { content?: string; type?: string; tags?: string[]; importance?: number; confidence?: number }) {
  const memory = db.prepare('SELECT * FROM memories WHERE id = ? AND deleted_at IS NULL').get(id) as MemoryRow | undefined;
  if (!memory) return { error: 'Memory not found', id };
...
function searchMemories(query: string, limit = 10, confidenceThreshold = 0) {
...
function getStats() {
  ...
  const entityRelations = (db.prepare('SELECT COUNT(*) as count FROM entity_relations').get() as { count: number }).count;
  return { 
    total, 
...
// ============================================================================
function createEdge(fromId: string, toId: string, relationType: string, validFrom?: string, validTo?: string, confidence = 1.0, metadata: Record<string, unknown> = {}) {
  ...
  if (!fromExists) return { error: `Source not found: ${fromId}` };
  if (!toExists) return { error: `Target not found: ${toId}` };
...
function queryEdges(memoryId?: string, direction: 'outgoing' | 'incoming' | 'both' = 'both', relationTypes?: string[], asOfDate?: string, includeExpired = false, limit = 50) {
  ...
  if (relationTypes?.length) { sql += ` AND relation_type IN (${relationTypes.map(() => '?').join(',')})`; params.push(...relationTypes); }
  if (asOfDate) { sql += ' AND valid_from <= ? AND (valid_to IS NULL OR valid_to > ?)'; params.push(asOfDate, asOfDate); }
  else if (!includeExpired) { sql += ' AND valid_to IS NULL'; }
  sql += ' ORDER BY valid_from DESC LIMIT ?'; params.push(limit);
  
  return (db.prepare(sql).all(...params) as EdgeRow[]).map(e => ({ ...e, metadata: JSON.parse(e.metadata) }));
}
...
function invalidateEdge(edgeId: string, validTo?: string) {
  ...
  if (edge.valid_to) return { error: 'Already invalidated', invalidatedAt: edge.valid_to };
...
// ============================================================================
function spreadingActivation(seedIds: string[], maxHops = 3, decayFactor = 0.5, inhibitionThreshold = 1.0, minActivation = 0.1, asOfDate?: string) {
  const activations = new Map<string, { activation: number; depth: number; path: string[]; edgeTypes: string[] }>();
  for (const id of seedIds) activations.set(id, { activation: 1.0, depth: 0, path: [id], edgeTypes: [] });
  const frontier = seedIds.map(id => ({ id, depth: 0, activation: 1.0, path: [id], edgeTypes: [] as string[] }));
  
  while (frontier.length > 0) {
    const curr = frontier.shift()!;
    if (curr.depth >= maxHops || curr.activation < minActivation) continue;
    ...
    for (const e of edges) {
      ...
      if (curr.path.includes(neighbor)) continue;
      const newAct = perEdge * e.confidence;
      if (newAct < minActivation) continue;
  ...
  for (const [id, data] of activations) {
    ...
    if (m) results.push({ ...toConfidentMemory(m), activation: data.activation, depth: data.depth, path: data.path, edgeTypes: data.edgeTypes });
  }
  return results.sort((a: any, b: any) => b.activation - a.activation);
}

// ============================================================================
// Working Memory / Scratchpad
// ============================================================================
function scratchSet(key: string, value: string, ttlSeconds?: number) {
...
function scratchGet(key: string) {
  const row = db.prepare('SELECT * FROM scratchpad WHERE key = ?').get(key) as ScratchRow | undefined;
...
function scratchList() {
  ...
  const rows = db.prepare('SELECT key, expires_at, created_at FROM scratchpad').all() as { key: string; expires_at: string | null; created_at: string }[];
  return rows;
}
 ...
 */
function createEntity(name: string, entityType = 'concept', observations: string[] = []) {
  validateEntityName(name);
  const validObs = validateObservations(observations);
  
  // Check if entity exists
  const existing = db.prepare('SELECT * FROM entities WHERE name = ?').get(name) as EntityRow | undefined;
...
function getEntity(name: string) {
  const entity = db.prepare('SELECT * FROM entities WHERE name = ?').get(name) as EntityRow | undefined;
  if (!entity) return { error: 'Entity not found', name };
  
  // Get outgoing relations
  const outgoing = db.prepare('SELECT * FROM entity_relations WHERE from_entity = ?').all(name) as EntityRelationRow[];
  // Get incoming relations
  const incoming = db.prepare('SELECT * FROM entity_relations WHERE to_entity = ?').all(name) as EntityRelationRow[];
 ...
 */
function createEntityRelation(from: string, to: string, relationType: string) {
 ...
 */
function searchEntities(query: string, entityType?: string, limit = 20) {
  const sanitizedQuery = sanitizeLikePattern(query);
  
  let sql = `SELECT * FROM entities WHERE (name LIKE ? ESCAPE '\\' OR observations LIKE ? ESCAPE '\\')`;
  const params: (string | number)[] = [`%${sanitizedQuery}%`, `%${sanitizedQuery}%`];
 ...
 */
function addObservations(name: string, observations: string[]) {
  const entity = db.prepare('SELECT * FROM entities WHERE name = ?').get(name) as EntityRow | undefined;
  if (!entity) return { error: 'Entity not found', name };
 ...
 */
function deleteEntity(name: string) {
...
const TOOLS = [
  ...
  { name: 'memory_stats', description: 'Get statistics including entity counts', inputSchema: { type: 'object' as const, properties: {} }},
  ...
  { name: 'memory_scratch_clear', description: 'Clear all scratchpad', inputSchema: { type: 'object' as const, properties: {} }},
  { name: 'memory_scratch_list', description: 'List scratchpad keys', inputSchema: { type: 'object' as const, properties: {} }},

  // Session (1 tool)
  { name: 'memory_briefing', description: 'Generate session briefing with memories and entities', inputSchema: {
    type: 'object' as const, properties: { maxTokens: { type: 'number', default: 500 } }
  }},
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      // Core memory
      case 'memory_store': result = storeMemory(a.content!, a.type, a.tags, a.importance, a.confidence); break;
      case 'memory_recall': result = recallMemory(a.id!); break;
      case 'memory_update': result = updateMemory(a.id!, { content: a.content, type: a.type, tags: a.tags, importance: a.importance, confidence: a.confidence }); break;
      case 'memory_search': result = searchMemories(a.query!, a.limit, a.confidenceThreshold); break;
      ...
      // Edges (for memories)
      case 'memory_edge_create': result = createEdge(a.fromId!, a.toId!, a.relationType!, a.validFrom, a.validTo, a.confidence, a.metadata); break;
      case 'memory_edge_query': result = queryEdges(a.memoryId, a.direction, a.relationTypes, a.asOfDate, a.includeExpired, a.limit); break;
      case 'memory_edge_invalidate': result = invalidateEdge(a.edgeId!, a.validTo); break;
      // Graph traversal
      case 'memory_graph_traverse': result = spreadingActivation(a.seedIds!, a.maxHops, a.decayFactor, a.inhibitionThreshold, a.minActivation, a.asOfDate).slice(0, a.limit || 20); break;
      // Scratchpad
      case 'memory_scratch_set': result = scratchSet(a.key!, a.value!, a.ttlSeconds); break;
      case 'memory_scratch_get': result = scratchGet(a.key!); break;
      case 'memory_scratch_delete': result = scratchDelete(a.key!); break;
      case 'memory_scratch_clear': result = scratchClear(); break;
      case 'memory_scratch_list': result = scratchList(); break;
      // Briefing
      case 'memory_briefing': result = generateBriefing(a.maxTokens); break;
      // NEW: Entity operations
      case 'memory_entity_create': result = createEntity(a.name!, a.entityType, a.observations); break;
      case 'memory_entity_get': result = getEntity(a.name!); break;
      case 'memory_entity_link': result = createEntityRelation(a.from!, a.to!, a.relationType!); break;
      case 'memory_entity_search': result = searchEntities(a.query!, a.entityType, a.limit); break;
      case 'memory_entity_observe': result = addObservations(a.name!, a.observations!); break;
      case 'memory_entity_delete': result = deleteEntity(a.name!); break;

...
interface ContradictionResult {
  id: string;
  content: string;
  contradictionType: 'semantic' | 'factual' | 'negation' | 'temporal' | 'antonym';
  confidence: number;
  similarity: number;
  explanation: string;
...
try { db.exec('ALTER TABLE memories ADD COLUMN project_id TEXT DEFAULT \'global\''); } catch {}
try { db.exec('ALTER TABLE edges ADD COLUMN project_id TEXT DEFAULT \'global\''); } catch {}
try { db.exec('ALTER TABLE scratchpad ADD COLUMN project_id TEXT DEFAULT \'global\''); } catch {}
try { db.exec('ALTER TABLE entities ADD COLUMN project_id TEXT DEFAULT \'global\''); } catch {}
try { db.exec('ALTER TABLE entity_relations ADD COLUMN project_id TEXT DEFAULT \'global\''); } catch {}
...
try { db.exec(`ALTER TABLE memories ADD COLUMN sentiment TEXT DEFAULT 'neutral'`); } catch {}
try { db.exec(`ALTER TABLE memories ADD COLUMN emotion_intensity REAL DEFAULT 0.0`); } catch {}
try { db.exec(`ALTER TABLE memories ADD COLUMN emotion_labels TEXT DEFAULT '[]'`); } catch {}
try { db.exec(`ALTER TABLE memories ADD COLUMN user_mood TEXT`); } catch {}

// Index for emotion-based queries
try { db.exec(`CREATE INDEX IF NOT EXISTS idx_memories_sentiment ON memories(sentiment)`); } catch {}
try { db.exec(`CREATE INDEX IF NOT EXISTS idx_memories_emotion_intensity ON memories(emotion_intensity)`); } catch {}
...
function validateObservations(observations: any[]): string[] {
...
function getEffectiveProject(projectId?: string): string {
  if (projectId && projectId.trim()) return projectId.trim().toLowerCase();
...
// ============================================================================
function calculateEffectiveConfidence(memory: any): number {
...
function assessConfidence(memory: any): { level: string; note?: string } {
...
function toConfidentMemory(m: any) {
 ...
 */
function hasNegation(text: string): { has: boolean; type: 'explicit' | 'implicit' | 'none'; word?: string } {
 ...
 */
function findAntonymConflict(text1: string, text2: string): { found: boolean; pair?: [string, string] } {
 ...
 */
function factsContradict(fact1: ExtractedFact, fact2: ExtractedFact): boolean {
 ...
 */
function findContradictions(content: string, projectId: string, limit = 5, excludeId?: string) {
  ...
  const scored = allMemories.map((m: any) => {
    const mWords = m.content.toLowerCase().split(/\W+/).filter((w: string) => w.length > 3);
...
// ============================================================================
async function storeMemory(content: string, type = 'note', tags: string[] = [], importance = 0.5, confidence = 0.5, projectId?: string) {
...
function recallMemory(id: string) {
  const memory = db.prepare('SELECT * FROM memories WHERE id = ? AND deleted_at IS NULL').get(id) as any;
...
async function updateMemory(id: string, updates: any) {
  const memory = db.prepare('SELECT * FROM memories WHERE id = ? AND deleted_at IS NULL').get(id) as any;
  if (!memory) return { error: 'Memory not found', id };
...
function listMemories(projectId?: string, limit = 20, includeDeleted = false) {
  ...
  const memories = db.prepare(sql).all(project, limit) as any[];
...
async function semanticSearch(query: string, projectId: string, limit: number, confidenceThreshold: number) {
...
async function hybridSearch(query: string, projectId: string, limit = 10, confidenceThreshold = 0): Promise<any[]> {
...
function getConfidentMemories(confidenceThreshold = 0.5, projectId?: string, limit = 20) {
  const project = getEffectiveProject(projectId);
...
// ============================================================================
async function findContradictionsProactive(content: string, projectId?: string, limit = 10) {
  const project = getEffectiveProject(projectId);
...
function invalidateEdge(edgeId: string) {
...
function scratchGet(key: string, projectId?: string) {
  const project = getEffectiveProject(projectId);
...
function scratchClear(projectId?: string) {
  const project = getEffectiveProject(projectId);
...
function createEntity(name: string, entityType = 'concept', observations: string[] = [], projectId?: string) {
  ...
  try {
    ...
    if (err.message.includes('UNIQUE')) {
      const existing = db.prepare('SELECT * FROM entities WHERE project_id = ? AND name = ?').get(project, name) as any;
...
function getEntity(name: string, projectId?: string) {
  const project = getEffectiveProject(projectId);
  const entity = db.prepare(`
    SELECT * FROM entities WHERE name = ? AND (project_id = ? OR project_id = 'global')
    ORDER BY CASE WHEN project_id = ? THEN 0 ELSE 1 END
    LIMIT 1
  `).get(name, project, project) as any;
  ...
  `).all(entity.name, entity.name, project) as any[];
...
function deleteEntity(name: string, projectId?: string) {
  const project = getEffectiveProject(projectId);
  const entity = db.prepare('SELECT * FROM entities WHERE name = ? AND project_id = ?').get(name, project) as any;
 ...
 */
function getTypeAncestors(typeName: string): string[] {
 ...
 */
function getTypeDescendants(typeName: string): string[] {
 ...
 */
function getTypeHierarchy(typeName: string) {
 ...
 */
function isSubtypeOf(childType: string, parentType: string): boolean {
 ...
 */
function recordAccessSequence(memoryIds: string[], sessionId: string, context?: string, projectId?: string) {
  if (!memoryIds || memoryIds.length === 0) return { recorded: false };
 ...
 */
function getSuggestions(memoryIds: string | string[], projectId?: string, limit = 10) {
  const project = getEffectiveProject(projectId);
  const ids = Array.isArray(memoryIds) ? memoryIds : [memoryIds];

  if (ids.length === 0) return { suggestions: [], reason: 'No memory IDs provided' };
  ...
  const suggestions = db.prepare(`
    ...
    LIMIT ?
  `).all(...ids, ...ids, project, limit) as any[];
 ...
 */
function getAccessPatterns(projectId?: string, limit = 20) {
  const project = getEffectiveProject(projectId);
  ...
  return {
    ...
    stats: {
      totalCoAccessRecords: (db.prepare('SELECT COUNT(*) as count FROM memory_coaccess WHERE project_id = ?').get(project) as any).count,
      totalSequences: (db.prepare('SELECT COUNT(*) as count FROM memory_access_sequences WHERE project_id = ?').get(project) as any).count
    }
  };
}
...
function suggestFromContext(contextText: string, projectId?: string, limit = 10) {
  ...
  const memories = db.prepare(`
    ...
    LIMIT ?
  `).all(project, ...params, limit) as any[];
...
function setEmotionalContext(
  ...
  // Validate sentiment
  const validSentiment = VALID_SENTIMENTS.includes(sentiment as Sentiment) ? sentiment : 'neutral';
 ...
 */
function getEmotionalContext(memoryId: string, projectId?: string) {
  const project = getEffectiveProject(projectId);
 ...
 */
function getEmotionStats(projectId?: string) {
  const project = getEffectiveProject(projectId);
 ...
 */
function findSimilarMemories(projectId?: string, similarityThreshold = 0.85, limit = 20): any[] {
  const project = getEffectiveProject(projectId);
 ...
 */
function strengthenActiveMemories(projectId?: string): number {
  const project = getEffectiveProject(projectId);
 ...
 */
function applyMemoryDecay(projectId?: string): number {
  const project = getEffectiveProject(projectId);
 ...
 */
function cleanExpiredScratchpad(projectId?: string): number {
  const project = getEffectiveProject(projectId);
 ...
 */
function runConsolidation(projectId?: string): any {
  const project = getEffectiveProject(projectId);
 ...
 */
function getConsolidationStatus(projectId?: string): any {
  const project = getEffectiveProject(projectId);
 ...
 */
function createCausalRelationship(
  causeId: string,
  effectId: string,
  ...
  const project = getEffectiveProject(projectId);
  const id = `causal_${randomUUID().slice(0, 8)}`;

  // Verify both memories exist
  const cause = db.prepare(`SELECT id, content FROM memories WHERE id = ? AND deleted_at IS NULL`).get(causeId) as any;
  const effect = db.prepare(`SELECT id, content FROM memories WHERE id = ? AND deleted_at IS NULL`).get(effectId) as any;

  if (!cause) return { error: 'Cause memory not found', causeId };
  if (!effect) return { error: 'Effect memory not found', effectId };
 ...
 */
function getCausalChain(memoryId: string, direction: 'upstream' | 'downstream' | 'both' = 'both', depth = 3, projectId?: string) {
  const project = getEffectiveProject(projectId);
  const visited = new Set<string>();

  function traverseUpstream(id: string, currentDepth: number): any[] {
  ...
  function traverseDownstream(id: string, currentDepth: number): any[] {
    if (currentDepth >= depth || visited.has(id)) return [];
    visited.add(id);
  ...
  const memory = db.prepare(`SELECT id, content FROM memories WHERE id = ? AND deleted_at IS NULL`).get(memoryId) as any;
  if (!memory) return { error: 'Memory not found', memoryId };
 ...
 */
function createAlternativeOutcome(
  causalId: string,
  ...
  const project = getEffectiveProject(projectId);
  const id = `alt_${randomUUID().slice(0, 8)}`;

  // Verify causal relationship exists
  const causal = db.prepare(`SELECT * FROM causal_relationships WHERE id = ?`).get(causalId) as any;
  if (!causal) return { error: 'Causal relationship not found', causalId };
 ...
 */
function recordActualOutcome(alternativeId: string, actualOutcome: string) {
  const alt = db.prepare(`SELECT * FROM alternative_outcomes WHERE id = ?`).get(alternativeId) as any;
  if (!alt) return { error: 'Alternative not found', alternativeId };
 ...
 */
function whatIf(query: string, projectId?: string, limit = 10) {
  const project = getEffectiveProject(projectId);
  ...
  const causalMatches = db.prepare(`
    ...
    LIMIT ?
  `).all(project, `%${sanitizeLikePattern(query)}%`, `%${sanitizeLikePattern(query)}%`, limit) as any[];
 ...
 */
function getCounterfactualStats(projectId?: string) {
  const project = getEffectiveProject(projectId);
 ...
 */
function parseNaturalTime(expression: string): Date | null {
 ...
 */
function getNextCronRun(cronExpr: string, from = new Date()): Date | null {
  ...
  const parseField = (expr: string, min: number, max: number): number[] => {
    ...
    if (expr.includes(',')) return expr.split(',').map(n => parseInt(n));
 ...
 * Reschedule a task
 */
function rescheduleTask(taskId: string, newSchedule: string) {
  const task = db.prepare(`SELECT * FROM scheduled_tasks WHERE id = ?`).get(taskId) as any;
  if (!task) return { error: 'Task not found', taskId };
 ...
 */
function extractTemporalReferences(text: string): string[] {
  const refs: string[] = [];
 ...
 */
function temporalRefsConflict(refs1: string[], refs2: string[]): { conflict: boolean; reason?: string } {
 ...
 */
function createResolutionRequest(memoryId1: string, memoryId2: string, projectId?: string) {
  const project = getEffectiveProject(projectId);

  // Check both memories exist
  const m1 = db.prepare(`SELECT id, content FROM memories WHERE id = ? AND deleted_at IS NULL`).get(memoryId1) as any;
  const m2 = db.prepare(`SELECT id, content FROM memories WHERE id = ? AND deleted_at IS NULL`).get(memoryId2) as any;

  if (!m1) return { error: 'Memory 1 not found', memoryId1 };
  if (!m2) return { error: 'Memory 2 not found', memoryId2 };
  ...
  if (existing) {
    return { error: 'Resolution already exists', resolutionId: (existing as any).id };
  }
 ...
 */
function getContradictionStats(projectId?: string) {
  const project = getEffectiveProject(projectId);
 ...
 */
function autoDeprecateContradicted(confidenceThreshold = 0.2, minContradictions = 3, projectId?: string) {
  const project = getEffectiveProject(projectId);
  ...
  `).all(project, confidenceThreshold, minContradictions) as any[];
 ...
 */
function detectIntentFromText(text: string): { type: string; confidence: number; keywords: string[] }[] {
 ...
 */
function recordIntentSignal(signalType: string, signalData: any, intentId?: string, projectId?: string) {
  const project = getEffectiveProject(projectId);
  const id = `sig_${randomUUID().slice(0, 8)}`;
 ...
 */
function inferIntentFromPatterns(projectId?: string): any {
  const project = getEffectiveProject(projectId);
  ...
  const recentAccesses = db.prepare(`
    ...
    JOIN memories m ON m.id IN (SELECT value FROM json_each(s.memory_ids))
...
function createIntent(
  ...
  const project = getEffectiveProject(projectId);
  const id = `intent_${randomUUID().slice(0, 8)}`;
 ...
 * Get active intents
 */
function getActiveIntents(projectId?: string, limit = 20) {
  const project = getEffectiveProject(projectId);
 ...
 */
function suggestForIntent(intentType: string, context?: string, projectId?: string, limit = 10) {
  const project = getEffectiveProject(projectId);
...
function completeIntent(intentId: string) {
  const intent = db.prepare(`SELECT * FROM user_intents WHERE id = ?`).get(intentId) as any;
  if (!intent) return { error: 'Intent not found', intentId };
 ...
 */
function predictIntent(contextText: string, projectId?: string) {
  const project = getEffectiveProject(projectId);
  ...
  for (const detected of detectedIntents.slice(0, 3)) {
    const existing = db.prepare(`
      ...
      LIMIT 1
    `).get(detected.type, project) as any;
...
function cleanupOldBackups(keepCount = 10) {
  if (!existsSync(BACKUP_DIR)) return { removed: [], kept: 0 };
...
function getStats(projectId?: string) {
  ...
  const memoryCounts = db.prepare(`SELECT COUNT(*) as total, SUM(CASE WHEN deleted_at IS NULL THEN 1 ELSE 0 END) as active FROM memories ${whereClause}`).get() as any;
  const entityCount = db.prepare(`SELECT COUNT(*) as count FROM entities ${whereClause}`).get() as any;
  const edgeCount = db.prepare(`SELECT COUNT(*) as count FROM edges ${whereClause}`).get() as any;
  const avgConfidence = db.prepare(`SELECT AVG(confidence) as avg FROM memories ${whereClauseDeleted}`).get() as any;
  const contradictionEdges = db.prepare(`SELECT COUNT(*) as count FROM edges WHERE relation_type LIKE 'contradiction_%' ${project ? `AND (project_id = '${project}' OR project_id = 'global')` : ''}`).get() as any;
  const withEmbeddings = db.prepare(`SELECT COUNT(*) as count FROM memories ${whereClauseDeleted} AND embedding IS NOT NULL`).get() as any;
  
  const typeBreakdown = db.prepare(`SELECT type, COUNT(*) as count FROM memories ${whereClauseDeleted} GROUP BY type ORDER BY count DESC`).all() as any[];
...
const server = new Server(
  { name: 'just-memory', version: '2.1.0' },
  { capabilities: { tools: {} } }
);

...
// ============================================================================
function findContradictions(content: string, limit = 5): MemoryRow[] {
  ...
  const allMemories = db.prepare('SELECT * FROM memories WHERE deleted_at IS NULL').all() as MemoryRow[];
...
function searchMemories(query: string, limit = 10, confidenceThreshold = 0) {
  const rows = db.prepare(`SELECT * FROM memories WHERE deleted_at IS NULL AND content LIKE ? ORDER BY confidence DESC, importance DESC LIMIT ?`)
    .all(`%${query}%`, limit * 2) as MemoryRow[];
  return rows

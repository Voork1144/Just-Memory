...
interface ActivatedMemory {
  id: string;
  content: string;
  activation: number;      // 0-1, how strongly activated
  depth: number;           // Hops from seed
  path: string[];          // How we got here
  edgeTypes: string[];     // Relationship types traversed
}
...
interface ToolArgs {
  ...
  edgeId?: string;
  memoryId?: string;
  // Spreading activation args (NEW in v1.3)
  seedId?: string;
  seedIds?: string[];
  maxHops?: number;
  decayFactor?: number;
...
function getStats() {
  ...
  const expiredEdges = (db.prepare('SELECT COUNT(*) as count FROM edges WHERE valid_to IS NOT NULL').get() as { count: number }).count;
...
function queryEdges(
  memoryId?: string, direction: 'outgoing' | 'incoming' | 'both' = 'both',
  relationTypes?: string[], asOfDate?: string, includeExpired = false, limit = 50
) {
...
function spreadingActivation(
  ...
  maxHops = 3,
  decayFactor = 0.5,
  inhibitionThreshold = 1.0,  // Lateral inhibition: cap total activation
  minActivation = 0.1,
  asOfDate?: string
): ActivatedMemory[] {
  ...
  while (frontier.length > 0) {
    ...
    for (const edge of edges) {
      ...
      if (existing) {
        ...
        const combinedActivation = Math.min(
...
const TOOLS = [
  ...
  { name: 'memory_stats', description: 'Get memory and edge statistics', inputSchema: { type: 'object' as const, properties: {} }},
    ...
    switch (name) {
      ...
      case 'memory_graph_traverse':
        result = spreadingActivation(
          a.seedIds!, a.maxHops || 3, a.decayFactor || 0.5,
          a.inhibitionThreshold || 1.0, a.minActivation || 0.1, a.asOfDate
        ).slice(0, a.limit || 20);
        break;

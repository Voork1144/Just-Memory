/**
 * Just-Memory v1.6 - P0 Security & CRUD Fixes
 * 
 * Fixes from v1.5:
 * - P0 FIX #1: SQL injection protection in search (sanitizeLikePattern)
 * - P0 FIX #2: Content length limits (MAX_CONTENT_LENGTH = 100KB)
 * - P0 FIX #3: Added memory_update tool for editing existing memories
 * 
 * Complete tool set: 20 tools (19 from v1.5 + memory_update)
 */
...
// ============================================================================
// Constants - P0 FIX #2: Content limits
// ============================================================================
...
// ============================================================================
// P0 FIX #1: SQL Injection Protection
// ============================================================================
...
// P0 FIX #3: Add memory_update function
function updateMemory(id: string, updates: { content?: string; type?: string; tags?: string[]; importance?: number; confidence?: number }) {
  ...
  if (updates.content !== undefined) {
    ...
    // Re-check contradictions with new content
    const contradictions = findContradictions(updates.content, id);
    if (contradictions.length > 0) {
      // Create new contradiction edges
      for (const c of contradictions) {
...
// P0 FIX #1: Sanitize LIKE pattern in search
...
function getStats() {
  ...
  return { total, byConfidenceLevel: byConfidence, activeEdges: edges, scratchpadItems: scratch, dbPath: DB_PATH, version: '1.6.0' };
}
...
// MCP Server Setup
// ============================================================================
const server = new Server({ name: 'just-memory', version: '1.6.0' }, { capabilities: { tools: {} } });

const TOOLS = [
  // Core memory (7 tools - added memory_update)
  ...
  // P0 FIX #3: New memory_update tool
...
async function main() {
  ...
  console.error('Just-Memory v1.6 running (P0 fixes: SQL injection, content limits, memory_update)');
}

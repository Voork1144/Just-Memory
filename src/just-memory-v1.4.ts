...
interface MemoryRow {
  ...
  confidence: number;           // NEW: Memory confidence score
  source_count: number;         // NEW: Number of confirming sources
  contradiction_count: number;  // NEW: Number of contradicting facts
}
...
interface ConfidentMemory {
  ...
  uncertaintyNote?: string;      // Acknowledgment of uncertainty
...
interface ToolArgs {
  ...
  // Confidence args (NEW in v1.4)
  confidenceThreshold?: number;
  sourceId?: string;            // Source that confirms this memory
  contradictingId?: string;     // Memory that contradicts this one
 ...
 */
function assessConfidence(memory: MemoryRow): {
 ...
 */
function toConfidentMemory(memory: MemoryRow): ConfidentMemory {
...
function searchMemories(query: string, limit = 10, confidenceThreshold = 0) {
  const rows = db.prepare(`
    ...
    LIMIT ?
  `).all(`%${query}%`, limit * 2) as MemoryRow[];  // Fetch extra to filter
...
function listMemories(limit = 20, includeWeak = false, includeUncertain = false) {
  ...
  return rows
    ...
    .filter(m => {
      if (!includeWeak && m.retention <= 0.1) return false;
      if (!includeUncertain && m.confidenceLevel === 'uncertain') return false;
      return true;
    })
    .slice(0, limit);
}
...
function getStats() {
  ...
  const edgeCount = (db.prepare('SELECT COUNT(*) as count FROM edges WHERE valid_to IS NULL').get() as { count: number }).count;
...
function createEdge(fromId: string, toId: string, relationType: string, validFrom?: string, validTo?: string, confidence = 1.0, metadata: Record<string, unknown> = {}) {
  ...
  if (!fromExists) return { error: `Source memory not found: ${fromId}` };
  if (!toExists) return { error: `Target memory not found: ${toId}` };
...
function invalidateEdge(edgeId: string, validTo?: string) {
  ...
  if (edge.valid_to) return { error: 'Edge already invalidated', invalidatedAt: edge.valid_to };
...
function spreadingActivation(seedIds: string[], maxHops = 3, decayFactor = 0.5, inhibitionThreshold = 1.0, minActivation = 0.1, asOfDate?: string) {
  ...
  while (frontier.length > 0) {
    ...
    if (current.depth >= maxHops || current.activation < minActivation) continue;
...
const TOOLS = [
  ...
  { name: 'memory_stats', description: 'Get statistics including confidence distribution', inputSchema: { type: 'object' as const, properties: {} }},

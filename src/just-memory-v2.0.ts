...
try { db.exec('CREATE INDEX IF NOT EXISTS idx_memories_project ON memories(project_id)'); } catch {}
try { db.exec('CREATE INDEX IF NOT EXISTS idx_edges_project ON edges(project_id)'); } catch {}
try { db.exec('CREATE INDEX IF NOT EXISTS idx_scratchpad_project ON scratchpad(project_id)'); } catch {}
try { db.exec('CREATE INDEX IF NOT EXISTS idx_entities_project ON entities(project_id)'); } catch {}
try { db.exec('CREATE INDEX IF NOT EXISTS idx_entity_relations_project ON entity_relations(project_id)'); } catch {}
...
async function storeMemory(content: string, type = 'note', tags: string[] = [], importance = 0.5, confidence = 0.5, projectId?: string) {
  ...
  return {
    ...
    potentialContradictions: contradictions.map((c: any) => ({ id: c.id, content: c.content.slice(0, 100) }))
  };
}
...
function getEntity(name: string, projectId?: string) {
  ...
  let entity = db.prepare('SELECT * FROM entities WHERE project_id = ? AND name = ?').get(project, name) as any;
  if (!entity && project !== GLOBAL_PROJECT) {
    entity = db.prepare('SELECT * FROM entities WHERE project_id = ? AND name = ?').get(GLOBAL_PROJECT, name) as any;
  }
  
  if (!entity) return { error: 'Entity not found', name, project_id: project };
  
  // Get relations
  const outgoing = db.prepare('SELECT * FROM entity_relations WHERE project_id = ? AND from_entity = ?').all(entity.project_id, name) as any[];
  const incoming = db.prepare('SELECT * FROM entity_relations WHERE project_id = ? AND to_entity = ?').all(entity.project_id, name) as any[];
  ...
  return {
    ...
    relations: {
      outgoing: outgoing.map((r: any) => ({ to: r.to_entity, type: r.relation_type })),
      incoming: incoming.map((r: any) => ({ from: r.from_entity, type: r.relation_type }))
    },
...
function linkEntities(from: string, relationType: string, to: string, projectId?: string) {
  ...
  if (!fromEntity) return { error: `Entity '${from}' not found`, project_id: project };
  if (!toEntity) return { error: `Entity '${to}' not found`, project_id: project };
...
function searchEntities(query: string, entityType?: string, projectId?: string, limit = 20) {
  ...
  `;
  const params: any[] = [project, `%${sanitized}%`, `%${sanitized}%`];
...
function observeEntity(name: string, observations: string[], projectId?: string) {
  ...
  const entity = db.prepare('SELECT * FROM entities WHERE (project_id = ? OR project_id = ?) AND name = ?')
    .get(project, GLOBAL_PROJECT, name) as any;
...
function deleteEntity(name: string, projectId?: string) {
  ...
  const entity = db.prepare('SELECT * FROM entities WHERE project_id = ? AND name = ?').get(project, name) as any;
  if (!entity) return { error: 'Entity not found', name, project_id: project };
...
function scratchGet(key: string, projectId?: string) {
  ...
  // Try project-specific first, then global
  let row = db.prepare('SELECT * FROM scratchpad WHERE key = ? AND project_id = ?').get(key, project) as any;
  if (!row && project !== GLOBAL_PROJECT) {
    row = db.prepare('SELECT * FROM scratchpad WHERE key = ? AND project_id = ?').get(key, GLOBAL_PROJECT) as any;
  }
  
  if (!row) return { key, project_id: project, value: null, found: false };
...
function scratchList(projectId?: string) {
  ...
  const rows = db.prepare('SELECT key, expires_at, created_at FROM scratchpad WHERE project_id = ? OR project_id = ?')
    .all(project, GLOBAL_PROJECT) as any[];
...
function queryEdges(memoryId: string, direction: 'outgoing' | 'incoming' | 'both' = 'both', projectId?: string) {
  const project = getEffectiveProject(projectId);
  let edges: any[] = [];
...
function backupMemories(projectId?: string) {
  ...
  let memories: any[], entities: any[], edges: any[];
...
function restoreMemories(backupPath: string, mode: 'merge' | 'replace' = 'merge', targetProject?: string) {
...
function getStats(projectId?: string) {
  ...
  let memoryStats: any, entityStats: any, scratchStats: any;
  ...
  if (project) {
    ...
    entityStats = db.prepare('SELECT COUNT(*) as count FROM entities WHERE project_id = ? OR project_id = ?').get(project, GLOBAL_PROJECT) as any;
    scratchStats = db.prepare('SELECT COUNT(*) as count FROM scratchpad WHERE project_id = ? OR project_id = ?').get(project, GLOBAL_PROJECT) as any;
  } else {
    memoryStats = db.prepare(`
      SELECT 
        COUNT(*) as total,
    ...
    entityStats = db.prepare('SELECT COUNT(*) as count FROM entities').get() as any;
    scratchStats = db.prepare('SELECT COUNT(*) as count FROM scratchpad').get() as any;
  }
  ...
  return {
    ...
    memories: {
      ...
      by_type: Object.fromEntries(typeBreakdown.map((t: any) => [t.type, t.count]))
    },
...
const server = new Server(
  { name: 'just-memory', version: '2.0.0' },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  const args = rawArgs as any;  // Cast to any to handle dynamic tool arguments
